...
import { RoomContext } from '../failcontext'
...
export default props => {
	...
	useEffect(() => {
		let initRooms, formatData
		if (context) {
			initRooms = context.initRooms
			formatData = context.formatData
		}
		let roomsdata
		if (props.data) {
			if (props.data.allContentfulHotelrooms) {
				if (props.data.allContentfulHotelrooms.edges) {
					roomsdata = props.data.allContentfulHotelrooms.edges
				}
			}
		}
		...
		initRooms({ rooms, featuredRooms, sortedRooms: rooms, price: maxPrice, maxPrice })
	}, [])
	let sortedRooms, rooms, handleChange, handleRange, formData
	if (context) {
		sortedRooms = context.sortedRooms
		rooms = context.rooms
		handleChange = context.handleChange
		handleRange = context.handleRange
		formData = context.formData
	}
	let { price, minPrice, maxPrice, breakfast, pets, type, capacity } = formData
		...
		<React.Fragment>
			<Layout>
				<Box mx='auto' mt='2' justifyContent='center'>
					<Heading m='3' color={colors.blacks[9]} fontSize={[ 3, 3, 4 ]} fontFamily='sans' textAlign='center'>
						Find your place
					</Heading>
				</Box>
